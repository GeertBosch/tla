...
variables
    ...
    l1:
        delta := [ i \in 1 .. Len(txn) |->
          << txn[i][1], txn[i][2], PickValueOr(Last(Key(snapshot, txn[i][1])),0) >>];
        if (lastOpTime = 1) print <<"delta",  delta>>;
...
\* BEGIN TRANSLATION (chksum(pcal) = "71729330" /\ chksum(tla) = "e8267582")
\* Procedure variable snapshot of procedure ApplyOps at line 65 col 9 changed to snapshot_
\* Procedure variable opTime of procedure ApplyOps at line 66 col 9 changed to opTime_
\* Parameter txn of procedure Observe at line 49 col 41 changed to txn_
...
l1(self) == /\ pc[self] = "l1"
            /\ delta' = [delta EXCEPT ![self] =        [ i \in 1 .. Len(txn_[self]) |->
                                                << txn_[self][i][1], txn_[self][i][2], PickValueOr(Last(Key(snapshot[self], txn_[self][i][1])),0) >>]]
            /\ IF lastOpTime = 1
                  THEN /\ PrintT(<<"delta",  delta'[self]>>)
                  ELSE /\ TRUE
...
\* Last modified Fri Mar 18 08:56:51 EDT 2022 by bosch
